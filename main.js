...
// ======= Helpers =======
function saveRememberMe(email, pass, remember) {
  if(remember){
...
function populateRememberMe(emailEl, passEl, rememberEl){
...
function pageLogin(){
  ...
  populateRememberMe(emailEl, passEl, rememberEl);
  ...
  loginBtn?.addEventListener("click", () => {
    ...
    signInWithEmailAndPassword(auth, email, pass)
      .then(() => {
        ...
        saveRememberMe(email, pass, rememberEl.checked);
  ...
  forgotBtn?.addEventListener("click", () => {
    ...
    sendPasswordResetEmail(auth, email)
      .then(() => msgEl.textContent = "📩 Reset email sent!")
      .catch(e => msgEl.textContent = "❌ " + e.message);
  });
}
...
function pageDashboard(){
  ...
  menuBtn?.addEventListener("click", () => {
    if(sidebar) sidebar.style.left = sidebar.style.left==='0px'?'-250px':'0px';
  });
}
